...
//             return $htmls;
    ...
    public function incomeAndExpenseQueryByPage()
    {
        $_session_user = Session::get(USER_SEESION);
        if(empty($_session_user)){
            return $this->redirect("/login/login/index");
    ...
    public function notice()
    {
        ...
        if(empty($_session_user)){
            ...
            $_user_notice = new Notice();
            $_res = $_user_notice->AllNoticeQueryWithLimit();
    ...
    public function NoticeDelete($noticeID)
    {
        $_session_user = Session::get(USER_SEESION);
        if(empty($_session_user)){
            return $this->redirect("/login/login/index");
        ...
        else
        {
            ...
            $res["success"] = $noticeOBJ->NoticeDel($noticeID);
            return json_encode($res);
        }
    }
...
//             }
    ...
    public function log()
    {
        ...
        if(empty($_session_user)){
            ...
            $_res = $_user_log->UserLogQueryWithLimit($_user_id, $_start, $_end);
