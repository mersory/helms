$(function() {
	var url = "/public/index.php/backend/Adminopt/lockorForbidonUser";
	...
	$("#open_user_list")
			.on(
					"click",
					function() {
						if ($("input[name='row-check']").filter(":checked")
								.size() == 0) {
							alert("请先选择需要操作的账号");
						} else if (confirm("确定要解锁这些会员吗?")) {
							var userIds = [];
							$("input[name='row-check']").filter(":checked")
